...
def read_datasets(mode):
    ...
    if mode == "train":
        # for training
        image_root = os.path.join(properties.train_images_path)
        actual_label_root = os.path.join(properties.train_actual_labels_path)

        for image_name in os.listdir(image_root):
            image_path = os.path.join(image_root, image_name)
            actual_label_path = os.path.join(actual_label_root, 'gt_' + image_name.split('_')[1])
...
def loader(image_path,
    ...
    if mode == 'train':
        image = cv2.imread(image_path, cv2.IMREAD_ANYDEPTH | cv2.IMREAD_UNCHANGED)
        actual_label = cv2.imread(actual_label_path, cv2.IMREAD_ANYDEPTH | cv2.IMREAD_UNCHANGED)
    elif mode == 'valid':
